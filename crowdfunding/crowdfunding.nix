{ self, inputs, config, ... }: {
  ...
  let crowdfunding = self.internal-lib.make-haskell-nix-pkg {
  ...
  in {
    ...
    ## because of https://github.com/input-output-hk/haskell.nix/issues/767
    packages.crowdfunding = flake.packages."crowdfunding:lib:crowdfunding";
    devShells.crowdfunding =
      crowdfunding.shellFor { withHoogle = true; };
  };
}
